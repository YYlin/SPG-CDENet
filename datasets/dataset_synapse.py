...
class Synapse_dataset(Dataset):
    def __init__(self, base_dir, list_dir, split, img_size, norm_x_transform=None, norm_y_transform=None):
        ...
        # 第一阶段的结果
        first_stage_base = os.path.join('./dataset/', "Synapse_SwinUNet_Organ")
        if self.split == "train":
            self.organ_path = os.path.join(first_stage_base, "train_npz")
        else:
            self.organ_path = os.path.join(first_stage_base, "test_vol_h5")
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            # 加载organ区域
            organ_path = os.path.join(self.organ_path, slice_name + '.h5')
            organ = h5py.File(organ_path)['organ'][:]
            ...
            organ_path = os.path.join(self.organ_path, vol_name + '.h5')
            organ = h5py.File(organ_path)['organ'][:]
        ...
        if self.norm_x_transform is not None:
            ...
            sample['organ'] = self.norm_x_transform(sample['organ'].copy())
